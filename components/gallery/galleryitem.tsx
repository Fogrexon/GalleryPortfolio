...
export const GalleryItem: VFC<GalleryItemProps> = ({
  item: { name, tags, link, sourcecode, image, description, demo },
}) => {
  const imgSrc = image || "/gallery/noimage.png";
  const ref = useRef<HTMLDivElement>();
  const [boxStyle, setBoxStyle] = useState<{ [key: string]: string }>({
  ...
  const mouseMove = (e) => {
    ...
    setBoxStyle({
      ...
      transform: `perspective(500px) rotateX(${
        -Math.floor(boxY * 10 * 10) / 10
      }deg) rotateY(${Math.floor(boxX * 10 * 10) / 10}deg)`,
      zIndex: "1",
    });
  };
  const mouseLeave = () => {
    ...
    <>
      ...
      >
        <div className={style.details} onClick={preventBubbling}>
          ...
          <div key="tags">
            Tags :
            {tags.map((entry: string) => (
              <span className={style.tags} key={entry}>
                {entry}
              </span>
            ))}
          </div>
