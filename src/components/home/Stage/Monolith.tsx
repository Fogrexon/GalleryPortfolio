...
import { Color, Vector3, Euler, Mesh, TextureLoader, MeshStandardMaterial } from "three";
...
export const Monolith: VFC<MonolithProps> = ({position, rotation}) => {
  ...
  const monolith = useMemo<Mesh | null>(() => monolithOrigin.scene.children[0].clone() as Mesh || null, [monolithOrigin]);
  ...
  useEffect(() => {
    if (monolith) {
      ...
      (monolith.material as MeshStandardMaterial).emissive = new Color(0xffffff);
  ...
  return (
    <group position={position} rotation={rotation}>
      <EffectPlane visible={visible} position={[-0.6, 0, 0]} rotation={[0, -Math.PI * 0.5, 0]} scale={[4, 1, 1]} />
      ...
      <primitive object={monolith} onPointerOver={() => setVisible(true)} onPointerOut={() => setVisible(false)} receiveShadow castShadow />
    </group>
  )
}